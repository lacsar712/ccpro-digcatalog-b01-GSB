...
6. **概览页** — 工地数、探方数、文物总数、近 7 日新增文物数、按器物类型统计、按工地对比（点击行跳转该工地的探方筛选）

## 统计口径（日期）

- `GET /api/overview` 返回的 `last7DaysNewFinds`：按文物记录的 `created_at`（登记时间）统计，口径为**东八区（UTC+8）自然日**，范围为含今天在内的最近 7 天，即 `[今天-6天 00:00, 明天 00:00)`（UTC+8）。
- 日界在后端以固定 `+08:00` 偏移计算（`time.FixedZone`），**不依赖服务器或容器的本地时区设置**；`created_at` 由后端进程写入，查询边界换算到同一进程时区后比较，保证口径一致。
- `bySite` 数组按工地汇总：`siteId`、`siteName`、`findCount`（文物数）、`unitCount`（探方数），按文物数降序排列。
